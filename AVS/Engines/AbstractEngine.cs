...
public abstract class AbstractEngine : MonoBehaviour, IScuttleListener
{
        ...
        get
        {
            ...
            if (av.IsNull())
                throw new System.Exception(
                    $"AvsVehicle component not found on {gameObject.name}. Please ensure this engine is attached to a AvsVehicle or its parent.");
            return av;
        }
    }
    ...
    protected virtual bool CanMove() => MV.GetIsUnderwater() || CanMoveAboveWater;

    /// <summary>
    /// Determines if the vehicle can rotate.
    /// </summary>
    /// <returns>True if rotation is allowed.</returns>
    protected virtual bool CanRotate() => MV.GetIsUnderwater() || CanRotateAboveWater;
    ...
    {
        var scalarFactor = 1.0f;
        var basePowerConsumptionPerSecond = moveDirection.x + moveDirection.y + moveDirection.z;
        var upgradeModifier = Mathf.Pow(0.85f, MV.NumEfficiencyModules);
        MV.PowerManager.TrySpendEnergy(scalarFactor * basePowerConsumptionPerSecond * upgradeModifier *
                                       Time.fixedDeltaTime);
    }
    ...
    public virtual void ControlRotation()
    {
        if (CanRotate())
        {
            var pitchFactor = 1.4f;
            var yawFactor = 1.4f;
            var mouseDir = GameInput.GetLookDelta();
            var xRot = mouseDir.x;
            var yRot = mouseDir.y;
    ...
    {
        var isForward = move.z != 0;
        var isRight = move.x != 0;
        var isUp = move.y != 0;
        var activated = isForward || isRight || isUp || MV.worldForces.IsAboveWater();
    ...
    public virtual void ExecutePhysicsMove()
    {
        ...
        RB.AddForce(tsm.z * MV.transform.forward * (ForwardMomentum / 100f) * Time.fixedDeltaTime,
            ForceMode.VelocityChange);
        RB.AddForce(tsm.x * MV.transform.right * (RightMomentum / 100f) * Time.fixedDeltaTime,
            ForceMode.VelocityChange);
    ...
    {
        var timeToXStop = Mathf.Log(0.05f * StrafeMaxSpeed / RightMomentum) / Mathf.Log(.25f);
        var timeToYStop = Mathf.Log(0.05f * VerticalMaxSpeed / UpMomentum) / Mathf.Log(.25f);
        var timeToZStop = Mathf.Log(0.05f * ForwardTopSpeed / ForwardMomentum) / Mathf.Log(.25f);
