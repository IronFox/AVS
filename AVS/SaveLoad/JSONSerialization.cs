...
{
    private readonly struct Handler(Func<object?, JToken> toJson, Func<JToken, object?> fromJson)
    {
        // ReSharper disable once MemberHidesStaticFromOuterClass
        public Func<object?, JToken> ToJson { get; } = toJson;

        // ReSharper disable once MemberHidesStaticFromOuterClass
        public Func<JToken, object?> FromJson { get; } = fromJson;
    }

    private static Dictionary<Type, Handler> Handlers { get; } = new()
    {
        {
        ...
        },
        {
            typeof(Vector3?), new Handler(
                ...
                token => token.Type == JTokenType.Null
                    ? null
                    : new Vector3(
                        token["x"]?.Value<float>() ?? 0,
                        token["y"]?.Value<float>() ?? 0,
        ...
        },
        {
            typeof(DateTimeOffset?), new Handler(
                ...
                token => token.Type == JTokenType.Null
                    ? null
                    : DateTimeOffset.Parse(token.Value<string>().Substring(3)))
        },
        {
            typeof(DateTimeOffset), new Handler(
        ...
        },
        {
            typeof(DateTime?), new Handler(
                ...
                token => token.Type == JTokenType.Null
                    ? null
                    : DateTime.Parse(token.Value<string>().Substring(3)))
        },
        {
            typeof(DateTime), new Handler(
    ...
    internal static JToken ToJson(object? value, Type t)
    {
        ...
        if (nul.IsNotNull())
        {
            var hasValueProperty = t.GetProperty("HasValue");
            if (hasValueProperty.IsNull())
                throw new InvalidOperationException(
                    $"Type {t.Name} does not have a 'HasValue' property, cannot serialize as nullable type.");
    ...
    internal static object? FromJson(JToken value, Type t)
    {
        ...
        try
        {
            ...
            foreach (var prop in t.GetProperties())
                if (jobj.TryGetValue(prop.Name, out var token))
                {
