...
{
    private static Dictionary<string, Node> KnownRootNodes { get; } = new();
        ...
        if (parent.IsNotNull())
            if (parent.Modules.Count > 0)
                throw new InvalidOperationException(
                    $"CraftTreeHandler: Cannot add a folder to a path that already contains modules. Path: {parent.GetPath()}");
