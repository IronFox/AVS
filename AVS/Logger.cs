...
public static class Logger
{
    ...
    {
        var result = $"<color=#FF0000>{prefix} Error: </color><color=#FFFF00>{message}</color>";
    ...
    {
        var result = $"<color=#FF0000>{prefix} Warning: </color><color=#FFFF00>{message}</color>";
        Notifications.Add(result);
        Error(message);
    }
    ...
    {
        yield return new WaitUntil(() => ErrorMessage.main.IsNotNull());
        yield return new WaitForSeconds(1);
        yield return new WaitUntil(() => ErrorMessage.main.IsNotNull());
        var eMain = ErrorMessage.main;
        var messageDuration = eMain.timeFlyIn + eMain.timeDelay + eMain.timeFadeOut + eMain.timeInvisible + 0.1f;
        while (Player.main.IsNull())
        {
