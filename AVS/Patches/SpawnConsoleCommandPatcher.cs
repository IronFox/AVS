...
public static class SpawnConsoleCommandPatcher
{
    ...
    {
        if (n.IsNotNull() && n.data.IsNotNull() && n.data.Count > 0)
        {
    ...
    private static void FinishAnySpawningVehicles()
    {
        ...
        AvsVehicleManager.VehiclesInPlay
            .Where(x => x.IsNotNull() && x.GetComponentInChildren<VFXConstructing>(true).IsNotNull() &&
                        x.GetComponentInChildren<VFXConstructing>(true).constructed < 100f)
    ...
    private static IEnumerator CheckSpawnForMVs(TechType tt)
    {
        ...
        if (result.IsNotNull() && result.GetComponent<AvsVehicle>().IsNotNull())
        {
