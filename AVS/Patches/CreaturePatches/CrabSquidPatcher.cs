...
internal class CrabSquidEnergyMixinPatcher
{
    /// <summary>
    ...
    {
        if (__instance.gameObject.GetComponentInParent<AvsVehicle>().IsNotNull())
        {
...
internal class CrabSquidPatcher
{
    /// <summary>
    ...
    {
        var maybeMV = collider.gameObject.GetComponentInParent<AvsVehicle>();
        if (maybeMV.IsNotNull())
        {
            maybeMV.GetComponent<EnergyInterface>().DisableElectronicsForTime(__instance.disableElectronicsTime);
            __instance.ApplyAndForgetOverlayFX(maybeMV
                .gameObject); // TODO: not sure if this is the right GameObject to target
