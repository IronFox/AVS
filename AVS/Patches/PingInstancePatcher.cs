...
public class PingInstancePatcher
{
    ...
    {
        if (__instance.origin.IsNull())
        {
            __instance.origin = __instance.transform;
            Logger.Warn(
                $"Found null origin for ping instance on object: {__instance.name}. Setting origin to itself. Otherwise, some ping sprites would not be displayed.");
        }
    }
}