...
public abstract class AvsVehicleModule : ILogFilter, INullTestableType
{
    ...
    public Node Node =>
        node ?? throw new InvalidOperationException(
            $"Cannot access {nameof(AvsVehicleModule)}.{nameof(Node)} before it has been registered");
    ...
    private List<AvsVehicleModule> RecipeExtensions { get; } = new();
    ...
    {
        if (this.node.IsNotNull() && this.node != node)
            throw new InvalidOperationException(
                $"Trying to reset {nameof(AvsVehicleModule)}.{nameof(Node)} from {this.node} to {node}");
        this.node = node;
    }
