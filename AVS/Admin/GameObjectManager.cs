...
public static class GameObjectManager<T> where T : Component
{
    ...
    private static List<T> AllSuchObjects { get; } = new();
    ...
    public static T? FindNearestSuch(Vector3 target, Func<T, bool>? filter = null)
    {
        ...
        var filtered = Where(x => x.IsNotNull());
        if (filter is not null)
            filtered = filtered.Where(filter);
        return filtered.OrderBy(ComputeDistance).FirstOrDefault();
    }
    ...
    {
        SanitizeList();
        return AllSuchObjects.Where(pred);
    }
